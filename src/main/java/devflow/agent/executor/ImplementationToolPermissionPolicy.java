 ...
 * <p>工具和执行器不再各自藏一份权限规则。
 */
final class ImplementationToolPermissionPolicy {

    private static final String ALLOWED_TOOLS_KEY = "devflow.implementation.allowed-tools";

    ImplementationToolPermissionContext build(
            Path projectPath,
            Set<Path> ownedPaths,
    ...
    boolean isToolVisible(ImplementationToolSpecification specification, ImplementationToolPermissionContext context) {
    ...
    void assertToolVisible(ImplementationToolSpecification specification, ImplementationToolPermissionContext context) {
    ...
    void assertWritablePath(Path relativePath, ImplementationToolPermissionContext context) {
    ...
    long resolveShellTimeout(Long requestedTimeoutMs, ImplementationToolPermissionContext context) {
        if (context == null) {
    ...
    ShellCommandDecision decideShellCommand(
            String command,
