 ...
 * 让后续编辑切回普通代码文件主链。
 */
class InlineScriptExtractToFileStrategy {

    private final HtmlPreciseEditor htmlPreciseEditor;

    InlineScriptExtractToFileStrategy(HtmlPreciseEditor htmlPreciseEditor) {
        this.htmlPreciseEditor = htmlPreciseEditor;
    }

    boolean shouldExternalize(Path htmlPath, EditUnit unit, GenerationFailureReport report) {
    ...
    GeneratedFileOutput externalize(Path htmlPath, String existingHtml, String scriptContent) {
