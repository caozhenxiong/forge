 ...
 */
final class CodeFilePatchExecutor {
    ...
    GeneratedFileOutput generate(FilePatchRouteRequest request) {
        if (request.patchProgressState() != null
                && request.patchProgressState().matches(request.relativePath(), FileEditStrategyNames.PRECISE_CODE)) {
