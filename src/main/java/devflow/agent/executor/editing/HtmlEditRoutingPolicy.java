 ...
 */
public final class HtmlEditRoutingPolicy {
    ...
    public EmbeddingAdapter<InlineScriptEditPlan> inlineScriptAdapter() {
        return htmlInlineScriptEmbeddingAdapter;
    }

    public EmbeddingAdapter<InlineStyleEditPlan> inlineStyleAdapter() {
        return htmlInlineStyleEmbeddingAdapter;
    }

    public boolean shouldFallbackToFocusedHtmlRegion(GenerationFailureReport report) {
        return report != null
                && report.usesPreciseEditingStrategy()
                && HOST_HTML_REGION_FAILURES.contains(report.failureType());
    }

    public boolean shouldFallbackFromInlineScript(GenerationFailureReport report) {
        return report != null && EMBEDDED_PATCH_FALLBACK_FAILURES.contains(report.failureType());
    }

    public boolean shouldFallbackFromInlineStyle(GenerationFailureReport report) {
        return report != null && EMBEDDED_PATCH_FALLBACK_FAILURES.contains(report.failureType());
    }
