 ...
 */
class FileEditStrategyResolver {

    private final HtmlPreciseEditor htmlPreciseEditor;
    private final LanguageEditAdapter codeEditAdapter;

    FileEditStrategyResolver(HtmlPreciseEditor htmlPreciseEditor, LanguageEditAdapter codeEditAdapter) {
        this.htmlPreciseEditor = htmlPreciseEditor;
        this.codeEditAdapter = codeEditAdapter;
    }

    boolean shouldUseStructuredHtmlDocumentGeneration(Path projectPath, Path relativePath, String existingContent) {
     ...
     */
    boolean mustUseLocalCodeEditing(Path projectPath, Path relativePath, DeliveryMode deliveryMode) {
        return codeEditAdapter.supports(relativePath)
                && deliveryMode != DeliveryMode.REWORK
                && (Files.exists(projectPath.resolve(relativePath))
     ...
     */
    boolean canUseWholeFileRewriteForCode(Path projectPath, Path relativePath, DeliveryMode deliveryMode) {
     ...
     */
    boolean canUseWholeFileRewriteForHtml(Path projectPath, Path relativePath, DeliveryMode deliveryMode) {
        if (!ProjectPathSupport.isHtml(relativePath)) {
            return true;
        }
        if (!Files.exists(projectPath.resolve(relativePath))) {
    ...
    boolean supportsLocalCodeEditing(Path relativePath, String existingContent) {
