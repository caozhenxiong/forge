 ...
 */
public final class FullRewritePromptAssembler {

    private FullRewritePromptAssembler() {
    }

    public static PatchGenerationPrompt assemble(
            Path relativePath,
        ...
        String user = """
                ...
                - 路径：%s
                - 变更原因：%s
                - 基线快照：exists=%s, sha256=%s, lineCount=%d
                ...
                relativePath,
                reason,
                fileSnapshot != null && fileSnapshot.exists(),
                fileSnapshot == null ? "" : fileSnapshot.contentHash(),
                fileSnapshot == null ? 0 : fileSnapshot.lineCount(),
