 ...
 * 不再揉回同一个编排层。
 */
public final class HostHtmlPatchExecutor {
    ...
    public String generateStructuredHtmlDocument(HtmlTargetedRewriteRequest request) {
        return structuredHtmlPatchExecutor.generate(request);
    }

    public String generatePreciseHtml(HtmlTargetedRewriteRequest request) {
        return preciseHtmlPatchExecutor.generate(request);
    }

    public String generateFocusedRegion(HtmlTargetedRewriteRequest request, HtmlEditRegion preferredRegion) {
